...
(def path-to-ddf-source (resolve-repo "ddf"))

;; ----------------------------------------------------------------------
;; # Karaf Container REPL Constants
;;
...
;; cp: ddf/libs/libs-pomfix/test/resources/features/target/tree.xml:
;; No such file or directory
(comment
  path-to-ddf-source
  (mvn-get-all-valid-subprojs path-to-ddf-source)
  (clojure.pprint/pprint (take 10 (file-seq (clojure.java.io/file path-to-ddf-source)))))

;; This takes forever to run; it should be replaced with an actual ddf build option to package
;; maven transitive metadata in the jar
(defn- mvn-save-deps
...
(comment
  (mvn-save-deps (str path-to-ddf-source "/platform/security/platform-security-core-api")))
...
(comment
  (generate-mvn-name (str path-to-ddf-source "/platform/security/platform-security-core-api"))
  (generate-mvn-name (str path-to-ddf-source "/platform/parser/xml")))
...
(comment
  (mvn-move-deps-file
    (str path-to-ddf-source "/platform/security/platform-security-core-api")
    my-output-dir)
  (mvn-move-deps-file
    (str path-to-ddf-source "/platform/parser/xml")
    my-output-dir))
...
(comment
  path-to-ddf-source
  (mvn-generate-deps-for-proj path-to-ddf-source))
...
(comment
  my-output-dir
  (mvn-move-deps-to path-to-ddf-source my-output-dir))
